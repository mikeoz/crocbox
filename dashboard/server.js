...
const server = http.createServer((req, res) => {
  ...
  if (pathname === '/api/config' && req.method === 'GET') {
    ...
    return res.end(JSON.stringify({
      ...
      crocLevel: env.CROC_LEVEL || 'beginner',
    }));
  }
  ...
  if (pathname === '/api/config/ai' && req.method === 'POST') {
    ...
    return;
  if (pathname === '/api/config/level' && req.method === 'POST') {
    let body = '';
    req.on('data', c => { body += c; });
    req.on('end', () => {
      try {
        const data = JSON.parse(body);
        const level = (data.value || 'beginner').toLowerCase();
        if (!['beginner', 'intermediate', 'expert', 'partner'].includes(level)) throw new Error('Invalid level');
        let envContent = '';
        try { envContent = fs.readFileSync(ENV_PATH, 'utf-8'); } catch {}
      ...
      try {
        ...
        const ALL_TOOLS = [
          ...
          },
          {
            ...
            description: 'Read a file from the user\'s machine. Use this when the user asks you to look at, read, summarize, or analyze a file on their system.',
            input_schema: {
              type: 'object',
              properties: { file_path: { type: 'string', description: 'The full path to the file to read' } },
              required: ['file_path']
            }
          },
          {
            name: 'filesystem_write',
            description: 'Write or create a file on the user\'s machine. Use this when the user asks you to save, create, or write content to a file.',
            input_schema: {
              type: 'object',
              properties: {
                file_path: { type: 'string', description: 'The full path where the file should be written' },
          ...
          {
            name: 'shell_exec',
            description: 'Run a command on the user\'s system. Use this when the user asks you to execute a command, run a script, or perform a system operation.',
            input_schema: {
              type: 'object',
        ...
        const TOOL_WHITELIST = {
          beginner:     ['web_search'],
          intermediate: ['web_search', 'filesystem_read'],
          expert:       ['web_search', 'filesystem_read', 'filesystem_write', 'shell_exec'],
          partner:      ['web_search', 'filesystem_read', 'filesystem_write', 'shell_exec'],
        };
        const crocLevel = (readEnv().CROC_LEVEL || 'beginner').toLowerCase();
        const allowedTools = TOOL_WHITELIST[crocLevel] || TOOL_WHITELIST.beginner;
        const tools = ALL_TOOLS.filter(t => allowedTools.includes(t.name));
        ...
        async function requestConsent(input) {
          return new Promise((resolve) => {
            const payload = typeof input === 'string'
              ? { type: 'api_call', target: 'api.duckduckgo.com', summary: `Search the web: "${input}"` }
              : input;
        ...
        // --- Tool executors (WP-3) ---
        async function executeFilesystemRead(filePath) {
          // Data Room scope check
          const room = dataRoomsApi.resolveRoom(filePath);
          if (!room) {
            return { success: false, content: 'This file is outside your Data Rooms. Access denied for safety.' };
          }
          if (room.closed) {
            return { success: false, content: `The "${room.name}" Data Room is closed. Access denied.` };
          }
          try {
            const stat = fs.statSync(filePath);
            if (stat.size > 1024 * 1024) {
              return { success: false, content: 'File is too large to read (over 1MB). Try a smaller file.' };
            }
            const content = fs.readFileSync(filePath, 'utf-8');
            return { success: true, content: content, source: 'filesystem' };
          } catch (err) {
            return { success: false, content: 'Could not read file: ' + err.message };
          }
        }

        async function executeFilesystemWrite(filePath, content) {
          // Data Room scope check — v0.7: Desktop only
          const room = dataRoomsApi.resolveRoom(filePath);
          if (!room) {
            return { success: false, content: 'This location is outside your Data Rooms. Write denied.' };
          }
          if (room.closed) {
            return { success: false, content: `The "${room.name}" Data Room is closed. Write denied.` };
          }
          try {
            const dir = path.dirname(filePath);
            if (!fs.existsSync(dir)) fs.mkdirSync(dir, { recursive: true });
            fs.writeFileSync(filePath, content, 'utf-8');
            return { success: true, content: `File written successfully to ${filePath}`, source: 'filesystem' };
          } catch (err) {
            return { success: false, content: 'Could not write file: ' + err.message };
          }
        }

        async function executeShellExec(command) {
          // Safety: no sudo, no shell chaining
          if (command.includes('sudo ')) {
            return { success: false, content: 'sudo commands are not permitted in CROCbox.' };
          }
          if (/[;&|]{1,2}/.test(command)) {
            return { success: false, content: 'Chained commands (&&, ||, ;) are not permitted in v0.7. Use a single command.' };
          }
          return new Promise((resolve) => {
            const { exec } = require('child_process');
            exec(command, { timeout: 30000, maxBuffer: 1024 * 512 }, (err, stdout, stderr) => {
              if (err) {
                resolve({ success: false, content: `Command failed: ${err.message}${stderr ? '\n' + stderr : ''}` });
              } else {
                resolve({ success: true, content: stdout || '(command produced no output)', source: 'shell' });
              }
            });
          });
        }

        // Build consent payload based on tool type
        function buildConsentPayload(tool) {
          switch (tool.name) {
            case 'web_search':
              return { type: 'api_call', target: 'api.duckduckgo.com', summary: `Search the web: "${tool.input.query}"` };
            case 'filesystem_read':
              return { type: 'filesystem', target: tool.input.file_path, summary: `Read file: ${tool.input.file_path}` };
            case 'filesystem_write':
              return { type: 'filesystem', target: tool.input.file_path, summary: `Write file: ${tool.input.file_path}` };
            case 'shell_exec':
              return { type: 'shell_exec', target: tool.input.command, summary: `Run command: ${tool.input.command}` };
            default:
              return { type: 'unknown', target: 'unknown', summary: `Unknown tool: ${tool.name}` };
          }
        }

        // Execute tool after consent granted
        async function executeTool(tool) {
          switch (tool.name) {
            case 'web_search':
              return executeSearch(tool.input.query);
            case 'filesystem_read':
              return executeFilesystemRead(tool.input.file_path);
            case 'filesystem_write':
              return executeFilesystemWrite(tool.input.file_path, tool.input.content);
            case 'shell_exec':
              return executeShellExec(tool.input.command);
            default:
              return { success: false, content: `Unknown tool: ${tool.name}` };
          }
        }

        // Build denial message based on tool type
        function buildDenialMessage(tool) {
          switch (tool.name) {
            case 'web_search':
              return 'Web access was blocked. Respond with genuine respect, then offer alternatives that do not require web access.';
            case 'filesystem_read':
              return 'File read was blocked by the user. Respond with genuine respect, then offer alternatives that do not require reading files.';
            case 'filesystem_write':
              return 'File write was blocked by the user. Respond with genuine respect, then offer alternatives that do not require writing files.';
            case 'shell_exec':
              return 'Command execution was blocked by the user. Respond with genuine respect, then offer alternatives that do not require running commands.';
            default:
              return 'Action was blocked. Respond with genuine respect.';
          }
        }
        ...
        for (let i = 0; i < 5; i++) {
          ...
          const consentPayload = buildConsentPayload(tool);
          webSearchQuery = consentPayload.summary;
          consentFired = true;
          currentMessages.push({ role: 'assistant', content: cr.content });
          // Data Room pre-check: if path is in a closed room, auto-deny
          if ((tool.name === 'filesystem_read' || tool.name === 'filesystem_write') && dataRoomsApi.isPathBlocked(tool.input.file_path)) {
            currentMessages.push({ role: 'user', content: [{ type: 'tool_result', tool_use_id: tool.id, content: buildDenialMessage(tool), is_error: true }] });
            ...
            return res.end(JSON.stringify({
              ...
              consentFired: true, denied: true, reason: 'room-closed',
              updatedHistory: [...currentMessages, { role: 'assistant', content: dr.content }]
            }));
          }
          // Gate 1: Request consent via proxy
          const decision = await requestConsent(consentPayload);
          ...
          // Gate 2: Consent granted — execute tool
          const toolResult = await executeTool(tool);
          currentMessages.push({ role: 'user', content: [{ type: 'tool_result', tool_use_id: tool.id, content: toolResult.content }] });
        }
  ...
  // Data Rooms API (WP-6)
  if (pathname === '/api/data-rooms' && req.method === 'GET') return dataRoomsApi.handleListRooms(req, res);
  if (pathname === '/api/data-rooms' && req.method === 'POST') return dataRoomsApi.handleAddRoom(req, res);
  if (pathname === '/api/data-rooms/close' && req.method === 'POST') return dataRoomsApi.handleCloseRoom(req, res);
